...
const ROOT = process.env.MEDPLUM_REACT_SRC ?? '/tmp/medplum-upstream/packages/react/src';
...
for (const dir of readdirSync(ROOT)) {
  ...
  for (const f of src) {
    ...
    loc += s.split('\n').length;
    for (const n of parseImports(s, '@mantine/core')) mantineCore.add(n);
    for (const n of parseImports(s, '@mantine/hooks')) mantineHooks.add(n);
    for (const pkg of ['@mantine/notifications', '@mantine/spotlight', '@mantine/dates', '@mantine/dropzone']) {
      for (const n of parseImports(s, pkg)) mantineOther.add(`${pkg.split('/')[1]}:${n}`);
    }
  ...
  for (const f of tests) testLoc += readFileSync(f, 'utf8').split('\n').length;
  for (const n of mantineCore) mantineTotals.set(n, (mantineTotals.get(n) ?? 0) + 1);
  rows.push({
    dir,
    srcFiles: src.length,
    loc,
