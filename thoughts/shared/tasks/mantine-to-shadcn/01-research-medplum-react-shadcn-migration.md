...
| Source | Pin | Notes |
|---|---|---|
| `medplum/medplum` upstream `main` | `8d589869f` ("Updating to include HH info (#10427)"), `@medplum/react@5.1.36` | Analyzed in a detached worktree. This is the behavioral reference. |
...
- `@mantine/notifications` `showNotification`/`updateNotification`/`notifications`: 17 files, ~49 call sites (top: `PharmacyDialog` 7, `BaseChat` 6, `ThreadInbox` 5, `AIRealTimeQuestionnaireForm` 4, `SignInForm` 4). One `toast` shim replaces them all.
