...
// Modified from @medplum/react 5.1.36 packages/react/src/ResourceInput/MultiResourceInput.tsx for @medplum/react-shadcn (Apache-2.0 §4(b) notice)
    ...
    return (
      <div ref={ref} {...others}>
        <div className="flex flex-nowrap items-center gap-2">
          ...
          <div>
            <div>{label}</div>
            <div className="text-xs text-muted-foreground">{(resource as Patient).birthDate || resource.id}</div>
          </div>
        </div>
      </div>
    );
...
function DefaultResourcePillComponent<T extends Resource>({
  item,
  disabled,
  onRemove,
}: {
  ...
  return (
    <div className="inline-flex items-center gap-1 rounded-full bg-secondary px-2 py-0.5 text-xs">
      <span>{item.label}</span>
      {!disabled && (
        <button type="button" aria-label={`Remove ${item.label}`} onClick={onRemove}>
          <IconX size={12} />
        </button>
      )}
    </div>
...
export function MultiResourceInput<T extends Resource = Resource>(
  ...
  return (
    <AsyncAutocomplete<T>
      ...
      pillComponent={DefaultResourcePillComponent}
