...
// Modified from @medplum/react 5.1.36 packages/react/src/ValueSetAutocomplete/ValueSetAutocomplete.tsx for @medplum/react-shadcn (Apache-2.0 §4(b) notice)
...
function combineNodes(first: ReactNode, second: ReactNode): ReactNode {
  if (first && second) {
    return (
      <>
        {first}
        <span className="mt-2 block">{second}</span>
      </>
    );
  }
  return first || second;
    ...
    return (
      <div ref={ref} {...others}>
        <div className="flex flex-nowrap items-center gap-2">
          ...
          <div>
            <span>{label}</span>
            <span className="block text-xs text-muted-foreground">{`${resource.system}#${resource.code}`}</span>
          </div>
        </div>
      </div>
    );
