...
// Modified from @medplum/react 5.1.36 packages/react/src/DiagnosticReportDisplay/DiagnosticReportDisplay.tsx for @medplum/react-shadcn (Apache-2.0 §4(b) notice)
...
export function DiagnosticReportDisplay(props: DiagnosticReportDisplayProps): JSX.Element | null {
  ...
  return (
    <div className="flex flex-col gap-4">
      <h1 className="text-2xl font-semibold tracking-tight">Diagnostic Report</h1>
      ...
      {diagnosticReport.conclusion && (
        <div className="mt-4 flex flex-col gap-4">
          ...
          <p>{diagnosticReport.conclusion}</p>
        </div>
      )}
...
function DiagnosticReportHeader({ value, hideSubject = false }: DiagnosticReportHeaderProps): JSX.Element {
  ...
  return (
    <div className="mt-4 flex flex-nowrap items-stretch gap-8">
      {hasLeftColumn && (
      ...
      {hasLeftColumn && hasRightColumn && <Separator orientation="vertical" />}
      {hasRightColumn && (
        <div className="flex flex-1 flex-col gap-4">
          ...
          {value.issued && (
            <HeaderField label="Issued">
              <p>{formatDateTime(value.issued)}</p>
            </HeaderField>
          )}
          {value.status && (
...
function HeaderField({ label, children }: HeaderFieldProps): JSX.Element {
  return (
    <div className="flex flex-nowrap items-start gap-4">
      <p className="w-[110px] shrink-0 text-muted-foreground">{label}</p>
      <div>{children}</div>
    </div>
  );
}
...
function PerformerDisplay({ value }: PerformerDisplayProps): JSX.Element {
  ...
  return (
    <>
      ...
      {address && (
        <p className="mt-1 ml-[34px] text-sm text-muted-foreground">
...
function SpecimenInfo(specimens: Specimen[] | undefined): JSX.Element {
  return (
    <div className="flex flex-col gap-2">
      <h2 className="text-sm font-semibold">Specimens</h2>

      <ol className="list-decimal">
        {specimens?.map((specimen) => (
          <li className="ml-3" key={`specimen-${specimen.id}`}>
            <div className="flex gap-5">
              <div className="flex gap-1.5">
                <p className="font-medium">Collected:</p> {formatDateTime(specimen.collection?.collectedDateTime)}
              </div>
              <div className="flex gap-1.5">
                <p className="font-medium">Received:</p> {formatDateTime(specimen.receivedTime)}
              </div>
            </div>
          </li>
        ))}
...
const tableClassName = cn(
  'w-full border-collapse border-[0.1px] border-gray-400 [&_td]:border-[0.1px] [&_td]:border-gray-400 [&_td]:p-1 [&_th]:border-[0.1px] [&_th]:border-gray-400 [&_th]:p-1',
  '[&_[data-slot=badge]]:min-w-max'
);
...
function ObservationRow(props: ObservationRowProps): JSX.Element | null {
  ...
  if (isGroupingObservation(observation)) {
    return (
      <>
        <TableRow className="bg-gray-100 font-semibold hover:bg-gray-100 dark:bg-zinc-800 dark:hover:bg-zinc-800">
          <TableCell colSpan={7} style={indentStyle(depth)} className="whitespace-normal">
        ...
        {displayNotes && (
          <TableRow>
            <TableCell colSpan={7} style={indentStyle(depth + 1)} className="whitespace-normal">
  ...
  return (
    <>
      <TableRow
        className={cn(
          critical &&
            'border-[0.1px] border-red-500 bg-red-50 font-medium text-red-500 hover:bg-red-50 dark:bg-red-900/20 dark:text-red-50 dark:hover:bg-red-900/20 [&>td]:border-[0.1px] [&>td]:border-red-500'
        )}
      >
        <TableCell rowSpan={displayNotes ? 2 : 1} style={indentStyle(depth)} className="whitespace-normal">
          <MedplumLink to={observation}>
            <CodeableConceptDisplay value={observation.code} />
          </MedplumLink>
        </TableCell>
        ...
        <TableCell>{observation.status && <StatusBadge status={observation.status} />}</TableCell>
      </TableRow>
      {memberRows}
      {displayNotes && (
        <TableRow>
...
function indentStyle(depth: number): { paddingInlineStart: string } | undefined {
  return depth > 0 ? { paddingInlineStart: `calc(4px + ${depth} * 1rem)` } : undefined;
}
...
function PerformingLabs({ value }: PerformingLabsProps): JSX.Element | null {
  ...
  return (
    <div className="mt-4 flex flex-col gap-2">
      <Separator />
      <p className="text-sm font-medium text-muted-foreground">Performing Labs</p>
      {labs.map((lab) => (
        <div key={`lab-${lab.id}-${lab.name}`} className="flex flex-nowrap items-start justify-between">
          <div>
            <p>{lab.name}</p>
            {lab.address?.[0] && (
...
function LabContactDisplay({ value }: LabContactDisplayProps): JSX.Element | null {
  ...
  return (
    <div className="text-right">
      {value.name && <p>{value.name.text ?? formatHumanName(value.name)}</p>}
      {value.telecom?.[0] && (
        <p className="text-sm text-muted-foreground">
