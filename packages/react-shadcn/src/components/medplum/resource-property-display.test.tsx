...
// Modified from @medplum/react 5.1.36 packages/react/src/ResourcePropertyDisplay/ResourcePropertyDisplay.test.tsx for @medplum/react-shadcn (Apache-2.0 §4(b) notice)
...
describe('ResourcePropertyDisplay', () => {
  ...
  describe('Secret field functionality', () => {
    test.skip('Renders secret field with masked value by default (Mantine-specific: asserts mantine-Flex-root class and inline Flex gap style)', async () => {
      await setup(
        <ResourcePropertyDisplay
      ...
      expect(parentElement).toHaveStyle({ 'align-items': 'center' } as Record<string, string>);
