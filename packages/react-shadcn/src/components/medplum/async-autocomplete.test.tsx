...
// Modified from @medplum/react 5.1.36 packages/react/src/AsyncAutocomplete/AsyncAutocomplete.test.tsx for @medplum/react-shadcn (Apache-2.0 §4(b) notice)
import type { AsyncAutocompleteOption } from '@/components/medplum/async-autocomplete';
...
import { act, fireEvent, render, screen, selectAutocompleteOption, typeInAutocomplete, within } from '@/test/render';
import type { JSX } from 'react';
import { StrictMode } from 'react';

vi.mock('@/lib/medplum/notify', () => ({ notify: { error: vi.fn() } }));
...
describe('AsyncAutocomplete', () => {
  ...
  test('a load failure that is not an abort surfaces a notification', async () => {
    ...
    expect(notify.error).toHaveBeenCalledWith(expect.stringContaining('Server exploded'));
  });
